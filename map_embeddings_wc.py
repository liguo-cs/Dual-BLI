...
def main():
    ...
    parser.add_argument('src_input_c', help='the input source context embeddings')
    parser.add_argument('trg_input', help='the input target embeddings')
    parser.add_argument('trg_input_c', help='the input target context embeddings')
    parser.add_argument('src_output', help='the output source embeddings')
    parser.add_argument('src_output_c', help='the output source context embeddings')
    parser.add_argument('trg_output', help='the output target embeddings')
    parser.add_argument('trg_output_c', help='the output target context embeddings')
    ...
    src_words, x = embeddings.read(srcfile, dtype=dtype, threshold=threshold)
    trg_words, z = embeddings.read(trgfile, dtype=dtype, threshold=threshold)

    srcfile_c = open(args.src_input_c, encoding=args.encoding, errors='surrogateescape')
    trgfile_c = open(args.trg_input_c, encoding=args.encoding, errors='surrogateescape')
    _, x_c = embeddings.read(srcfile_c, dtype=dtype, threshold=threshold)
    _, z_c = embeddings.read(trgfile_c, dtype=dtype, threshold=threshold)
    ...
    embeddings.normalize(x_c, args.normalize)
    embeddings.normalize(z, args.normalize)
    embeddings.normalize(z_c, args.normalize)
    ...
    xw = xp.empty_like(x)
    zw = xp.empty_like(z)
    x_cw = xp.empty_like(x_c)
    z_cw = xp.empty_like(z_c)
    ...
    simfwd_c = xp.empty((args.batch_size, trg_size), dtype=dtype)
    simbwd_c = xp.empty((args.batch_size, src_size), dtype=dtype)
    if args.validation is not None:
        simval = xp.empty((len(validation.keys()), z.shape[0]), dtype=dtype)
        simval_c = xp.empty((len(validation.keys()), z_c.shape[0]), dtype=dtype)
    ...
    src_indices_forward_c = xp.arange(src_size)
    trg_indices_forward_c = xp.zeros(src_size, dtype=int)
    ...
    src_indices_backward_c = xp.zeros(trg_size, dtype=int)
    trg_indices_backward_c = xp.arange(trg_size)
    knn_sim_fwd = xp.zeros(src_size, dtype=dtype)
    knn_sim_bwd = xp.zeros(trg_size, dtype=dtype)
    knn_sim_fwd_c = xp.zeros(src_size, dtype=dtype)
    knn_sim_bwd_c = xp.zeros(trg_size, dtype=dtype)
    ...
    while True:
        ...
        if args.orthogonal or not end:  # orthogonal mapping
            ...
            if args.whiten:
                ...
                xw = xw.dot(wx1)
                zw = zw.dot(wz1)
                wx1_c = whitening_transformation(x_cw[src_indices])
                wz1_c = whitening_transformation(z_cw[trg_indices])
                x_cw = x_cw.dot(wx1_c)
                z_cw = z_cw.dot(wz1_c)
            ...
            xw = xw.dot(wx2)
            zw = zw.dot(wz2)
            wx2_c, s_c, wz2_t_c = xp.linalg.svd(x_cw[src_indices].T.dot(z_cw[trg_indices]))
            wz2_c = wz2_t_c.T
            x_cw = x_cw.dot(wx2_c)
            z_cw = z_cw.dot(wz2_c)
        ...
        if end:
            ...
            if args.direction in ('forward', 'union'):
                if args.csls_neighborhood > 0:
                    for i in range(0, trg_size, simbwd.shape[0]):
                        ...
                        # CSLS for context embedding
                        z_cw[i:j].dot(x_cw[:src_size].T, out=simbwd_c[:j-i])
                        knn_sim_bwd_c[i:j] = topk_mean(simbwd_c[:j-i], k=args.csls_neighborhood, inplace=True)
                for i in range(0, src_size, simfwd.shape[0]):
                    j = min(i + simfwd.shape[0], src_size)
                    xw[i:j].dot(zw[:trg_size].T, out=simfwd[:j-i])
                    simfwd[:j-i].max(axis=1, out=best_sim_forward[i:j])
                    simfwd[:j-i] -= knn_sim_bwd/2  # Equivalent to the real CSLS scores for NN
                    a, mask = dropout(simfwd[:j-i], 1 - keep_prob)
                    a.argmax(axis=1, out=trg_indices_forward[i:j])
                    x_cw[i:j].dot(z_cw[:trg_size].T, out=simfwd_c[:j-i])
                    # simfwd_c[:j-i].max(axis=1, out=best_sim_forward[i:j])
                    simfwd_c[:j-i] -= knn_sim_bwd_c/2  # Equivalent to the real CSLS scores for NN
                    (simfwd_c*mask).argmax(axis=1, out=trg_indices_forward_c[i:j]) # drop the items from the same positions
            if args.direction in ('backward', 'union'):
                if args.csls_neighborhood > 0:
                    for i in range(0, src_size, simfwd.shape[0]):
                        ...
                        x_cw[i:j].dot(z_cw[:trg_size].T, out=simfwd_c[:j-i])
                        knn_sim_fwd_c[i:j] = topk_mean(simfwd_c[:j-i], k=args.csls_neighborhood, inplace=True)
                for i in range(0, trg_size, simbwd.shape[0]):
                    j = min(i + simbwd.shape[0], trg_size)
                    zw[i:j].dot(xw[:src_size].T, out=simbwd[:j-i])
                    simbwd[:j-i].max(axis=1, out=best_sim_backward[i:j])
                    simbwd[:j-i] -= knn_sim_fwd/2  # Equivalent to the real CSLS scores for NN
                    a, mask = dropout(simbwd[:j-i], 1 - keep_prob)
                    a.argmax(axis=1, out=src_indices_backward[i:j])
                    z_cw[i:j].dot(x_cw[:src_size].T, out=simbwd_c[:j-i])
                    # simbwd[:j-i].max(axis=1, out=best_sim_backward[i:j])
                    simbwd_c[:j-i] -= knn_sim_fwd_c/2  # Equivalent to the real CSLS scores for NN
                    (simbwd_c*mask).argmax(axis=1, out=src_indices_backward_c[i:j])
            if args.direction == 'forward':
                ...
                # take intersection of two dicitionaries
                index_forward = (trg_indices_forward == trg_indices_forward_c)
                index_backward = (src_indices_backward == src_indices_backward_c)
                src_indices = xp.concatenate((src_indices_forward[index_forward], src_indices_backward[index_backward]))
                trg_indices = xp.concatenate((trg_indices_forward[index_forward], trg_indices_backward[index_backward]))
                print('D size:', len(src_indices))
            ...
            if args.validation is not None:
                ...
                x_cw[src].dot(z_cw.T, out=simval_c)
                nnn = asnumpy(simval_c.argmax(axis=1))
                accuracy = np.mean([1 if nn[i] in validation[src[i]] else 0 for i in range(len(src))])
                accuracy_c = np.mean([1 if nnn[i] in validation[src[i]] else 0 for i in range(len(src))])
                # acc = np.mean([1 if nn[i] in validation[src[i]] and nn[i] == nnn[i] else 0 for i in range(len(src))])
                acc_D = np.sum([1 if nn[i] in validation[src[i]] and nn[i] == nnn[i] else 0 for i in range(len(src))]) / np.sum([1 if nn[i] == nnn[i] else 0 for i in range(len(src))])
                # wc_same = np.mean([1 if nn[i] == nnn[i] else 0 for i in range(len(src))])
            ...
            if args.verbose:
                ...
                if args.validation is not None:
                    ...
                    print('\t- Val. accuracy_c:  {0:9.4f}%'.format(100 * accuracy_c), file=sys.stderr)
                    print('\t- Val. accuracy_D:  {0:9.4f}%'.format(100 * acc_D), file=sys.stderr)
                    # print('\t- Val. same_right:  {0:9.4f}%'.format(100 * acc), file=sys.stderr)
                    # print('\t- Val. wc_same:     {0:9.4f}%'.format(100 * wc_same), file=sys.stderr)
                sys.stderr.flush()
            if args.log is not None:
                val = '{0:.6f}\t{1:.6f}\t{2:.6f}\t{3:.6f}\t{4:.6f}'.format(
                    100 * similarity, 100 * accuracy, 100 * validation_coverage, 100 * accuracy_c, 100 * acc_D) if args.validation is not None else ''
    ...
    srcfile_c = open(args.src_output_c, mode='w', encoding=args.encoding, errors='surrogateescape')
    trgfile_c = open(args.trg_output_c, mode='w', encoding=args.encoding, errors='surrogateescape')
    embeddings.write(src_words, xw, srcfile)
    embeddings.write(trg_words, zw, trgfile)
    embeddings.write(src_words, x_cw, srcfile_c)
    embeddings.write(trg_words, z_cw, trgfile_c)
    srcfile.close()
    trgfile.close()
    srcfile_c.close()
    trgfile_c.close()
